...
interface MyError {
  code: number;
  message: string;
  // any other properties your error object might have
}
...
const submitHandler = async (userCredential: IForm) => {
  ...
  try {
    if (userCredential.password_confirm && userCredential.name) {
      await createUserWithEmailAndPassword(auth, userCredential);
    }
    signInWithEmailAndPassword(auth, userCredential);
  } catch (error) {
    const myError = error as MyError;
    const errorCode: number = myError.code;
    const errorMessage: string = myError.message;
    console.log(myError);
...
<template>
  <div class="form hidden h-screen flex-col justify-start items-start mt-24">
    <div class="w-full flex justify-between items-center mb-4">
      <h1
        class="inline-block text-2xl sm:text-3xl font-extrabold text-primary tracking-tight dark:text-slate-200"
      >
        {{ registrationMode ? $t("signUp") : $t("enter") }}
      </h1>
      <button class="text-accent underlin underline-offset-4" @click="openSignUp">
        Go to {{ registrationMode ? $t("enter") : $t("signUp") }}
      </button>
    </div>
    <FormKit
      type="form"
    ...
    >
      ...
      <FormKit
        ...
        :help="registrationMode ? $t('emailHelp') : ''"
      ...
      <FormKit
        ...
        :help="registrationMode ? $t('passwordHelp') : ''"
      />
      <FormKit
        type="password"
      ...
      />
      <FormKit
        outer-class="primaryBtn p-0 w-full"
        type="submit"
        :label="registrationMode ? $t('signUp') : $t('enter')"
      />
    </FormKit>
    <AuthProvider
      class="authproviders"
